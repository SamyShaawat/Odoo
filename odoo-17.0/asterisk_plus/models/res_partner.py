...
# Skip searching for numbers less then MAX_EXTEN_LEN
...
class Partner(models.Model):
    ...
    def search_by_number(self, number):
        ...
        found = self.search([
            ...
            ('mobile_normalized', search_operation, number)])
    ...
    # Override https://github.com/odoo/odoo/blob/15.0/addons/phone_validation/models/res_partner.py#L21
    def _phone_format(self, number, country=None, company=None, force_format='E164'):
        disable = self.env['asterisk_plus.settings'].get_param('disable_phone_format')
        if disable:
            return strip_number(number)
        elif tools.odoo.release.version_info[0] < 16:
            return super()._phone_format(number, country=country, company=company)
        else:
            return super()._phone_format(number, country=country, company=company, force_format=force_format)
...
if tools.odoo.release.version_info[0] >= 17:
    ...
    class Partner(models.Model):
        ...
        def _phone_format(self, fname=False, number=False, country=False, force_format='E164', raise_exception=False):
            ...
            if disable:
                return strip_number(number)
            return super()._phone_format(fname=fname, number=number, country=country,
                force_format=force_format, raise_exception=raise_exception)
