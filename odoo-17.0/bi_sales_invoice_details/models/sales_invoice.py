...
class SaleOrderUpdate(models.Model):
	...
	paid_amount = fields.Float(String ='Paid Amount',compute ='_compute_amount_paid_2')
	...
	def _compute_invoice_amount(self):
		for record in self:
			invoice_id = self.env['account.move'].sudo().search(['&',('invoice_origin','=', record.name),'|',('state','=','draft'),('state','=','posted'),('payment_state', 'not in', ['reversed', 'invoicing_legacy'])])
			total = 0
	...
	def _compute_amount_due(self):
		for record in self:
			invoice_ids = self.env['account.move'].sudo().search(['&',('invoice_origin','=', record.name),'|',('state','=','draft'),('state','=','posted'),('payment_state', 'not in', ['reversed', 'invoicing_legacy'])])
			amount = 0
	...
	def _compute_amount_paid_2(self):
