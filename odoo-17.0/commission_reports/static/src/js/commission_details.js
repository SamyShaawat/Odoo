odoo.define('commission_reports.commission_details_report', function (require) {
"use strict";
...
var CommissionDetailsReport = AbstractAction.extend({
    hasControlPanel: true,

    events: {
        'click .js_report_commission_details_pdf': '_onPrintPdfBtnClicked',
        'click .js_report_commission_details_xlsx': '_onPrintXlsxBtnClicked',
    },
    ...
    get_html: async function() {
        const { html, options } = await this._rpc({
            args: [this.kwargs],
            method: 'get_html',
            model: 'commission.details.report',
        });
        console.log(options);
        this.html = html;
        this.kwargs = options;
    ...
    renderButtons: function() {
        var self = this;
        this.$buttons = $(QWeb.render("commission_reports.commission_details_buttons", {}));
        return this.$buttons;
    },

    renderSearchViewButtons: function () {
        var self = this;
        this.$searchview_buttons = $(QWeb.render("commission_reports.commission_details_searchview_buttons", {options:this.kwargs}));
        ...
        this.$searchview_buttons.find('.js_commission_report_search_btn').click(function (event) {
            ...
            var error_message = '';
            var filter_by = self.$searchview_buttons.find('.o_input[name="filter_by"]').val();
            if (filter_by){
                self.kwargs.filter_by = filter_by;
            }else{
                new WarningDialog(self, {
                    title: _t("Odoo Warning"),
                }, {
                    message: _t("Invalid filter by")
                }).open();
            }
            ...
            if (date_from.length > 0){
                ...
                self.kwargs.date_from = field_utils.parse.date(date_from.val());
    ...
    _onPrintPdfBtnClicked: function (){
        ...
        var vals = {
          ...
          "data": {
            "model": "commission.details.report",
            "output_format": "pdf",
            "report_name": "commission_details",
...
core.action_registry.add('commission_details_client', CommissionDetailsReport);

return CommissionDetailsReport;
