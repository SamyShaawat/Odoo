...
class WhatsappSendMessage(models.TransientModel):
    ...
    script = fields.Many2one('whatsapp.message.script')
    message_script = fields.Many2one('whatsapp.message.script.message')
    domain_script = fields.Char(compute='domain_script_func')

    message_box = fields.Text(string="Message box")
    profile_id = fields.Many2one('whatsapp.profile')
        ...
        for rec in self:
            ...
            if rec.message_box:
                response = {
                    ...
                    "recipientId": rec.profile_id.patient_whatsapp,
                    "response": [
                        {
                            "type": "TEXT",
