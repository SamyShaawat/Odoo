...
class AccountTaxReportHandler(models.AbstractModel):
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        options['buttons'].append(
            {'name': _('Closing Entry'), 'action': 'action_periodic_vat_entries', 'sequence': 110, 'always_show': True})
    ...
    def action_periodic_vat_entries(self, options):
        ...
        locked_companies_moves = self._get_tax_closing_entries_for_closed_period(report, options, tax_locked_companies,
                                                                                 posted_only=False)
        ...
        draft_closing_moves = self._get_tax_closing_entries_for_closed_period(report, options, non_tax_locked_companies,
                                                                              posted_only=False) \
                              + draft_locked_moves
        companies_to_regenerate = non_tax_locked_companies + draft_locked_moves.company_id
        moves += self._generate_tax_closing_entries(report, options, companies=companies_to_regenerate,
                                                    closing_moves=draft_closing_moves)
    ...
    def _generate_tax_closing_entries(self, report, options, closing_moves=None, companies=None):
        ...
        if closing_moves:
            ...
            for company in companies:
                ...
                company_closing_moves = company._get_and_update_tax_closing_moves(end_date,
        ...
        for company, company_closing_moves in closing_moves_by_company.items():
            ...
            for move in company_closing_moves:
                ...
                move_options = {**options,
                                'fiscal_position': move.fiscal_position_id.id if move.fiscal_position_id else 'domestic'}
    ...
    def _compute_vat_closing_entry(self, company, options):
        ...
        for tg, values in tax_groups.items():
            ...
            if not currency.is_zero(total):
                ...
                key = (tg.advance_tax_payment_account_id.id or False, tg.tax_receivable_account_id.id,
...
class GenericTaxReportCustomHandler(models.AbstractModel):
    ...
    def _get_dynamic_lines(self, report, options, grouping):
        ...
        if grouping in ('tax_account', 'account_tax'):
            tax_amount_hierarchy = self._read_generic_tax_report_amounts(report, options_by_column_group,
                                                                         groupby_fields)
        else:
            tax_amount_hierarchy = self._read_generic_tax_report_amounts_no_tax_details(report, options,
                                                                                        options_by_column_group)
    ...
    def _populate_lines_recursively(self, report, options, lines, sorting_map_list, groupby_fields, values_node,
                                    index=0, type_tax_use=None, parent_line_id=None):
        ...
        for key in sorted_keys:
            ...
            report_line = self._build_report_line(report, options, default_vals, groupby_key, sorting_map[key][0],
                                                  parent_line_id)
    ...
    def _build_report_line(self, report, options, default_vals, groupby_key, value, parent_line_id):
        ...
        if groupby_key == 'src_tax_type_tax_use':
            ...
            report_line['id'] = report._get_generic_line_id(None, None, markup=type_tax_use_option[0],
